 ...
 */
    ...
    $page->OC_HTML_setHeaderBar(array_merge($conf->_OC_conf_dirs ,
                         array("Re-Read Configs"=>"config.php?reset=reset&type=".urlencode("tool")."&display=".urlencode($dispnum))));
    ...
    if($_GET['file'])  // conf file requested via menu link
    {
        ...
        if($result) // file read successfully
        {
            ...
            {
                $menuList[$record[0]] = "$PHP_SELF?type=tool&display=configedit&file=" . $_GET['file'] . "&section=$record[0]";
            }
    ...
    else // default page
    {
        ...
        if($files)
        {
            ...
            {
                $menuList[$file] = "$PHP_SELF?type=tool&display=configedit&file=$file&section=$file";
                // HTML output
